...
If newly classified data points are added to the data set, then it will continually grow. As discussed in the [lazy learning](#lazy-learning) section, larger data sets increase size and slow down performance, and class outliers or mislabelled points can have negative effects on further classification, meaning that more data is not necessarily better.

For context, a class outlier is defined as a data point with one label, surrounded (in terms of the distance metric) by points with other labels. It may be itself mislabelled, or it may be correctly labelled but overlapping with another class. In any case, data points like this can cause classification issues.
